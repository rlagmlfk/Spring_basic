 ...
 * 사용자가 화면에 입력하는 값을 받아오기 위해서 공통 코드를 만듬
 * 오라클 테이블 컬럼의 수가 50개이면 req.getParameter()를 50번 써야 하는 것임
 * 산업과 관련된 테이블의 컬럼 수는 100개 이상 200개 이하 인 것도 다반사임
 * 따라서 스프링과 같은 프레임 워크에서는 반복되는 코드를 줄이기 위한 util기능의 클래스도 지원함
 */
...
public class HashMapBinder {
	...
	public void bind(Map<String,Object> pMap) {
		// 사용자가 입력한 값을 담을 맵이 외부 클래스에서 인스턴스화 되어 넘어오니까 초기화 처리
		pMap.clear(); // 초기화를 해줌
